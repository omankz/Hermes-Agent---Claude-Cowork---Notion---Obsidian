...
3. **整合層** — Claude Cowork 與 Notion 負責 GUI 操作任務及專案管理
...
> **本地優先原則：** 所有推論、記憶與 Wiki 操作皆在 RPi5 上運行，完全不依賴雲端服務。Claude Cowork 與 Notion 僅用於需要 GUI 介面或團隊協作的任務。
...
| **Claude Cowork** | 桌面自動化、GUI 任務 | PC | 您 + AI |
...
**預估處理時間：** 每篇論文約 2–5 分鐘。所有流程皆在 RPi5 本地端執行。
